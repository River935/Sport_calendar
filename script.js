...
function load() { //
  ...
  const paddingDays = weekdays.indexOf(dateString.split(', ')[0]); //padding days empty days before the month start
...
function initButtons() { //button for the next and back button
  ...
  document.getElementById('detailedEvent').addEventListener('click', showEvent);
